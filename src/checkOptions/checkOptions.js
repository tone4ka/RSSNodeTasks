...
async function checkOptions(options, errorHandler) {
  ...
  if (doubleOptionsNames(options))
    throw new myError("Error: You provided some argument more than once");
  if (missingCOption(options))
    throw new myError("Error: You should provide -c argument");
  ...
  if (outputFile)
    await checkOutputFile(outputFile, errorHandler).catch((err) => {
      errorHandler(err);
    });
}
